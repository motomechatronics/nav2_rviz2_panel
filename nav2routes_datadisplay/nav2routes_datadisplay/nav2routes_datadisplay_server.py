...
class Nav2RouterDataDisplayServer(Node):
    def __init__(self):
        super().__init__('nav2routes_datadisplay_node')
        self.get_logger().info("constructor calling...")
        self.srv_visualize_route = self.create_service(NavroutesServiceMessage, 'nav2routes_datadisplay_server', self.visualize_route_callback)   
        ...
        self.package_path = get_package_share_directory(self.package_name) + "/config/site_data"
...
def main(args=None):
    ...
    nav2routesviz_object = Nav2RouterDataDisplayServer()
    rclpy.spin(nav2routesviz_object)
